...
用户请求 → TradingAssistant (navigator.py)
    ↓
ContextAssembler (core/data/) → 组装 MarketContext
    ↓
├── 技术面分析 → IndicatorEngine (indicators/indicator_engine.py)
│   └── 自研35+指标 + TqSdk内置70+指标 + 7维趋势强度 (ER/R2/Hurst/ADX-ROC/EMA斜率/TSI/ATR比率)
├── 基本面分析 → NewsCrawler + SupplyDemandProvider + GeopoliticalTracker
│   └── 10+新闻源(国际权威+国内产业) + 供需数据 + 地缘政治风险追踪
└── 多维度筛选 → MultiDimensionScreener (indicators/multi_dimension_screener.py)
│   └── Z-score归一化 → MAD投票 → 加权评分
    ↓
