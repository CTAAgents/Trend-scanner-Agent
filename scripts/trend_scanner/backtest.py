...
class WalkForwardBacktester:
    ...
    def rolling_backtest(self, df: pd.DataFrame, strategy_func,
...
class MonteCarloValidator:
    ...
    def regime_analysis(self, returns: np.ndarray,
        ...
        for regime_id in np.unique(regimes):
            ...
            if len(regime_returns) > 0:
                regime_stats[int(regime_id)] = {
                    ...
                    'sharpe': round(float(np.mean(regime_returns) / (np.std(regime_returns) + 1e-10) * np.sqrt(252)), 3),
...
class ParameterOptimizer:
    ...
    def grid_search(self, df: pd.DataFrame, strategy_func,
