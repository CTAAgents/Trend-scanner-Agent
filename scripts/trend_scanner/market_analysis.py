...
class MultiIndicatorConsensus:
    ...
    def check_indicator_health(cls, df: pd.DataFrame) -> Dict:
        """检查指标健康度"""
        all_indicators = (cls.TREND_INDICATORS + cls.MA_INDICATORS +
                         cls.OSCILLATOR_INDICATORS + cls.MOMENTUM_INDICATORS)
    ...
    def _vote_trend_strength(cls, df: pd.DataFrame) -> Dict:
        ...
        if ema20 > 0 and ema60 > 0:
            ...
            if ma_trend_direction == 'STRONG_BULLISH':
                ...
                # 弱多头：多头排列 + 一升一降
                votes['weak'] += 3
                details.append(f"弱多头排列（一升一降）→弱(×3)")
                ...
                # 弱空头：空头排列 + 一升一降
                votes['weak'] += 3
                details.append(f"弱空头排列（一升一降）→弱(×3)")
            else:
                # 回退到原始逻辑（如果没有斜率数据）
                if ema20 > ema60:
                    # 多头排列
                    if ema20_slope > 1 and ema60_slope > 0:
                        ...
                        votes['weak'] += 3
                        details.append(f"多头排列+斜率向上→弱(×3)")
                    else:
                        votes['weak'] += 2
                        details.append(f"多头排列+斜率走平→弱(×2)")
                if ema20 > ema60:
                    # 空头排列
                    if ema20_slope < -1 and ema60_slope < 0:
                        votes['strong'] += 4
                        ...
                        votes['weak'] += 3
                        details.append(f"空头排列+斜率向下→弱(×3)")
                    else:
                        votes['weak'] += 2
                        details.append(f"空头排列+斜率走平→弱(×2)")
                else:
                    votes['none'] += 1
    ...
    def _vote_trend_direction(cls, df: pd.DataFrame) -> Dict:
        ...
        if 'dc_upper' in df.columns and 'dc_lower' in df.columns:
            ...
            if close > 0 and dc_upper > dc_lower:
                ...
                if close > dc_upper * 0.98:
                    votes['bullish'] += 2
                    details.append(f"价格接近/突破通道上轨→趋势延续(领先×2)")
                # 突破下轨
                elif close < dc_lower * 1.02:
                    votes['bearish'] += 2
                    details.append(f"价格接近/跌破通道下轨→趋势延续(领先×2)")
        ...
        if ema20 > 0 and ema60 > 0:
            if ma_trend_direction == 'STRONG_BULLISH':
                ...
                votes['bearish'] += 2
                details.append(f"伪多头排列（两者都在下降）→空(确认×2)")
            elif ma_trend_direction == 'WEAK_BULLISH':
                votes['bullish'] += 1
                details.append(f"弱多头排列（一升一降）→弱多(确认×1)")
            elif ma_trend_direction == 'STRONG_BEARISH':
                votes['bearish'] += 2
                details.append(f"真空头排列（斜率{ema20_slope:.2f}%）→空(确认×2)")
            elif ma_trend_direction == 'FALSE_BEARISH':
                votes['bullish'] += 2
                details.append(f"伪空头排列（两者都在上升）→多(确认×2)")
            elif ma_trend_direction == 'WEAK_BEARISH':
                votes['bearish'] += 1
                details.append(f"弱空头排列（一升一降）→弱空(确认×1)")
            else:
                if ema20 > ema60:
                    votes['bullish'] += 1
    ...
    def _vote_momentum(cls, df: pd.DataFrame) -> Dict:
            ...
            if not pd.isna(ema20_prev) and not pd.isna(ema60_prev):
                ...
                if len(df) >= 20:
                    ...
                    if close > recent_high:
                        votes['healthy'] += 1
                        details.append(f"突破20日高点→趋势延续(×1)")
                    elif close < recent_low:
                        votes['healthy'] += 1
                        details.append(f"跌破20日低点→趋势延续(×1)")
        ...
        ema20 = cls._safe_val(df.get('ema20'))
        ema60 = cls._safe_val(df.get('ema60'))
        strong_trend = adx > Config.ADX_STRONG_TREND or (ema20 > 0 and ema60 > 0 and (
            (ema20 > ema60 * 1.01) or (ema20 < ema60 * 0.99)))
        ...
        if bullish_oscillators >= Config.OSCILLATOR_RESONANCE:
            votes['healthy'] += 2
            details.append(f"RSI/STOCH/CCI共振偏多→动量强(×2)")
        elif bearish_oscillators >= Config.OSCILLATOR_RESONANCE:
            votes['healthy'] += 2
            details.append(f"RSI/STOCH/CCI共振偏空→动量强(×2)")
        elif bullish_oscillators >= 2:
            votes['healthy'] += 1
            details.append(f"2/3振荡器偏多→动量中等(×1)")
        elif bearish_oscillators >= 2:
            votes['healthy'] += 1
            details.append(f"2/3振荡器偏空→动量中等(×1)")
    ...
    @classmethod
    def _score_secondary_factors(cls, df: pd.DataFrame, direction_vote: Dict,
                                 momentum_vote: Dict) -> Dict[str, List[float]]:
        """
        二级因子打分系统
        ...
        is_strong_trend = adx > 25 and ma_trend in ('STRONG_BULLISH', 'STRONG_BEARISH',
                                                      'FALSE_BULLISH', 'FALSE_BEARISH')
    ...
    @classmethod
    def _smooth_factors(cls, df: pd.DataFrame, factors: Dict[str, List[float]],
                        alpha: float = 0.3) -> Dict[str, List[float]]:
        """
        因子平滑（EMA）
    ...
    @classmethod
    def _get_rolling_percentile(cls, df: pd.DataFrame, col: str,
                                percentile: float, window: int = 120) -> float:
        """
        获取滚动分位数（用于自适应阈值）
    ...
    @classmethod
    def _merge_correlated_factors(cls, factors: Dict[str, List[float]],
                                  threshold: float = 0.85) -> Dict[str, List[float]]:
        """
    ...
    def _calc_dimension_win_rates(cls, df: pd.DataFrame, dimension_scores: Dict) -> Dict:
        ...
        # 获取当前价格方向
        if len(df) < 2:
            return {dim: 0.5 for dim in dimension_scores.keys()}
    ...
    def consensus(cls, df: pd.DataFrame) -> Dict:
        ...
        if cold_start['is_cold_start']:
            return {
                ...
                'conflict_level': 'none',
                'evidence': [f'冷启动期：剩余{cold_start["bars_remaining"]}根K线，进度{cold_start["progress"]*100:.0f}%'],
                'warnings': ['冷启动期不产生交易信号'],
                'actionable': False,
                'votes': {'strength': {'strong': 0, 'weak': 0, 'none': 0},
        ...
        secondary_factors = cls._merge_correlated_factors(
            secondary_factors, Config.FACTOR_CORRELATION_THRESHOLD)
        ...
        if close_val > 0 and ema20 > 0 and ema60 > 0:
            ...
            if ma_gap_pct < Config.MA_CONSOLIDATION_THRESHOLD and adx_val < Config.MA_CONSOLIDATION_ADX_MAX:
                ...
                dimension_scores['trend_confirmation']['weighted'] = round(
                    dimension_scores['trend_confirmation']['score'] * dimension_scores['trend_confirmation']['weight'], 2)
        ...
        if filtered_dimensions:
            ...
            if total_weight > 0:
                filtered_composite = sum(
                    d['score'] * d['weight'] / total_weight
                    for d in filtered_dimensions.values()
                )
            else:
        ...
        # 收集证据
        evidence = []
        evidence.append(f"健康度: {health['health_score']:.0f}% ({health['available_count']}/{health['available_count']+health['missing_count']}指标)")
        ...
        if is_consolidation:
            evidence.append(f"⚠️ 均线粘合: EMA20/60差值<{Config.MA_CONSOLIDATION_THRESHOLD*100:.1f}%，排列权重衰减至{Config.MA_CONSOLIDATION_ARRANGEMENT_DECAY*100:.0f}%")
...
class TrendPhaseDetector:
    ...
    PHASES = ["CONSOLIDATING", "EMERGING", "DEVELOPING",
              "MATURE", "FATIGUING", "REVERSING"]
...
class MarketStateClassifier:
    ...
    @classmethod
    def set_thresholds(cls, adx_strong: int = 25, adx_weak: int = 20,
                       channel_period: int = 20, rsi_period: int = 14):
...
class LLMReasoningLayer:
    ...
    # 允许的状态值
    VALID_STATES = ["STRONG_UPTREND", "STRONG_DOWNTREND",
                    "WEAK_UPTREND", "WEAK_DOWNTREND", "RANGE_BOUND"]

    # 允许的阶段值
    VALID_PHASES = ["CONSOLIDATING", "EMERGING", "DEVELOPING",
                    "MATURE", "FATIGUING", "REVERSING"]
...
{{
    ...
    def classify_with_llm(self, df: pd.DataFrame) -> Dict:
        ...
        if not self.use_llm or self.llm_provider is None:
            return {'code_result': code_result, 'llm_result': None,
                    'final_result': code_result, 'used_llm': False}
        ...
        if llm_result is not None:
            ...
            if is_valid:
                return {'code_result': code_result, 'llm_result': llm_result,
                        'final_result': llm_result, 'used_llm': True}
            else:
