...
class MetaSkillEngine:
    ...
    def create_skill_from_template(self, template_id: str,
                                   custom_params: Dict[str, Any] = None) -> Optional[GeneratedSkill]:
        """
        从模板创建技能
    ...
    def refine_skill_from_failures(self, skill_id: str,
                                   failures: List[Dict[str, Any]]) -> Optional[SkillEvolution]:
        """
        根据失败信号精炼技能
    ...
    def _generate_improvements(self, skill: GeneratedSkill,
        ...
        if 'risk_mismanagement' in failure_types:
            # 风控失误：添加风控规则
            improvements['rule_changes'].append(
                "添加规则: 单笔风险不超过账户的2%"
            )
