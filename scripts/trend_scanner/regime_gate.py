...
REGIME_SIMILARITY_MATRIX = {
    'CONSOLIDATING': {'CONSOLIDATING': 1.0, 'EMERGING': 0.5, 'DEVELOPING': 0.2, 'MATURE': 0.1, 'FATIGUING': 0.2, 'REVERSING': 0.3},
    'EMERGING':      {'CONSOLIDATING': 0.5, 'EMERGING': 1.0, 'DEVELOPING': 0.8, 'MATURE': 0.3, 'FATIGUING': 0.2, 'REVERSING': 0.2},
    'DEVELOPING':    {'CONSOLIDATING': 0.2, 'EMERGING': 0.8, 'DEVELOPING': 1.0, 'MATURE': 0.7, 'FATIGUING': 0.3, 'REVERSING': 0.1},
    'MATURE':        {'CONSOLIDATING': 0.1, 'EMERGING': 0.3, 'DEVELOPING': 0.7, 'MATURE': 1.0, 'FATIGUING': 0.8, 'REVERSING': 0.3},
    'FATIGUING':     {'CONSOLIDATING': 0.2, 'EMERGING': 0.2, 'DEVELOPING': 0.3, 'MATURE': 0.8, 'FATIGUING': 1.0, 'REVERSING': 0.7},
    'REVERSING':     {'CONSOLIDATING': 0.3, 'EMERGING': 0.2, 'DEVELOPING': 0.1, 'MATURE': 0.3, 'FATIGUING': 0.7, 'REVERSING': 1.0},
}
...
class RegimeGate:
    ...
    def __init__(self, 
                 same_regime_boost: float = 1.5,
                 cross_regime_penalty: float = 0.5,
                 category_match_boost: float = 1.2):
        """
        初始化机制门
    ...
    def calculate_regime_weight(self, experience_phase: str, 
                                current_phase: str,
                                phase_confidence: float = 1.0) -> float:
        """
        计算机制权重
    ...
    def apply_regime_weights(self, matches: List[ExperienceMatch],
                            current_phase: str,
                            phase_confidence: float = 1.0) -> List[RegimeMatchResult]:
        """
        对经验匹配结果应用机制权重
        ...
        for match in matches:
            ...
            # 计算机制权重
            regime_weight = self.calculate_regime_weight(
                exp.trend_phase,
                current_phase,
                phase_confidence
            )
            ...
            result = RegimeMatchResult(
                ...
                is_same_category=(self.get_regime_category(exp.trend_phase) == 
                                 self.get_regime_category(current_phase)),
    ...
    def generate_regime_report(self, current_phase: str, 
                              results: List[RegimeMatchResult]) -> str:
        """
        生成机制权重报告
...
"""
    ...
    def generate_strategy_mix(self, results: List[RegimeMatchResult],
        ...
        if total_weight > 0:
            strategy_weights = {
                phase: round(weight / total_weight, 3)
                for phase, weight in phase_weights.items()
            }
        else:
            strategy_weights = {
                phase: 1.0 / len(phase_groups)
...
class RegimeAwareRetriever:
    ...
    def __init__(self, regime_gate: Optional[RegimeGate] = None):
    ...
    def retrieve_with_regime_awareness(
        ...
        # 应用机制权重
        results = self.regime_gate.apply_regime_weights(
            matches,
            current_phase,
            phase_confidence
        )
...
def create_regime_gate(same_regime_boost: float = 1.5,
                      cross_regime_penalty: float = 0.5) -> RegimeGate:
    """创建机制门实例"""
    return RegimeGate(
