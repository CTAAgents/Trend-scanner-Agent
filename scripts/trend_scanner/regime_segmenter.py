...
class RegimeSegmenter:
    ...
    def segment(self, df: pd.DataFrame,
                ...
                return_segments: bool = True) -> List[RegimeSegment]:
        """
        分割市场数据
    ...
    def _create_segments(self, df: pd.DataFrame,
                         change_points: List[int]) -> List[RegimeSegment]:
        """创建机制段"""
        segments = []
        start = 0
    ...
    def _compute_segment_stats(self, segment: RegimeSegment,
                               data: pd.DataFrame):
        """计算段的统计特征"""
        # 价格统计
...
class PhaseLabeler:
    ...
    def label(self, segment: RegimeSegment,
              data: pd.DataFrame) -> RegimeSegment:
        """
        对机制段进行语义标注
    ...
    def _compute_indicators(self, data: pd.DataFrame) -> Dict:
        ...
        indicators['ema60'] = close.ewm(span=60, adjust=False).mean().iloc[-1] if len(data) >= 60 else indicators.get('ema20', close.iloc[-1])
...
class HierarchicalRegimeDetector:
    ...
    def __init__(self,
                 segmenter: Optional[RegimeSegmenter] = None,
                 labeler: Optional[PhaseLabeler] = None):
        """
        初始化分层检测器
    ...
    def detect(self, df: pd.DataFrame,
               features: List[str] = None) -> List[RegimeSegment]:
        """
        执行分层机制检测
...
# 便捷函数
def create_segmenter(algorithm: str = "pelt",
                    min_size: int = 10) -> RegimeSegmenter:
    """创建分割器实例"""
    return RegimeSegmenter(algorithm=algorithm, min_size=min_size)


def create_hierarchical_detector(algorithm: str = "pelt",
                                min_size: int = 10) -> HierarchicalRegimeDetector:
    """创建分层检测器实例"""
...
def detect_regime_changes(df: pd.DataFrame,
                         features: List[str] = None,
                         algorithm: str = "pelt",
                         min_size: int = 10) -> List[RegimeSegment]:
    """快速检测机制变化"""
