...
class ScoringAnalytics:
    ...
    def calculate_correlation(self, symbol: str = None,
                            start_date: str = None,
    ...
    def calculate_correlation_by_state(self, market_state: str,
    ...
    def generate_report(self, symbol: str = None) -> str:
        ...
        # 按市场状态统计
        state_stats = {}
        for state in ['STRONG_UPTREND', 'WEAK_UPTREND', 'RANGE_BOUND',
        ...
        for state, stats in state_stats.items():
            if stats.get('insufficient_data'):
                ...
                report.append(f"| {state} | {stats['sample_size']} | "
                            f"{stats['accuracy']:.2%} | {stats['win_rate']:.2%} |")
        ...
        if dimension_effectiveness:
            ...
            report.append("|------|--------|------------|--------|----------|----------|")
            for dim_name, stats in sorted(dimension_effectiveness.items(),
                                          key=lambda x: abs(x[1].get('correlation', 0)),
                                          reverse=True):
                report.append(f"| {dim_name} | {stats['correlation']:.4f} | "
