...
class DecayConfig:
    ...
    # 相关机制定义
    related_regimes: Dict[str, List[str]] = field(default_factory=lambda: {
...
class SelectiveUpdater:
    ...
    def calculate_decay_factor(self, experience: Experience,
        ...
        try:
            exp_time = datetime.fromisoformat(
                experience.timestamp.replace('Z', '+00:00').split('+')[0]
            )
        ...
        # 2. 机制衰减
        regime_decay = self._calculate_regime_decay(
            experience.trend_phase, current_phase
        )
    ...
    def _calculate_regime_decay(self, experience_phase: str,
                               current_phase: str) -> float:
        """计算机制衰减"""
    ...
    def apply_decay_to_weights(self, experiences: List[Experience],
                               current_phase: str) -> List[Tuple[Experience, float]]:
        """
        对经验权重应用衰减
    ...
    def filter_by_decay(self, experiences: List[Experience],
                       current_phase: str,
                       min_decay: float = 0.1) -> List[Experience]:
        """
        根据衰减因子过滤经验
...
class KnowledgeDistiller:
    ...
    def _select_key_experiences(self, experiences: List[Experience],
                               n: int = 5) -> List[Experience]:
        """
        选择关键经验
        ...
        """
        # 按时间排序（最新）
        by_time = sorted(experiences, 
                        key=lambda x: x.timestamp, reverse=True)
        
        # 按收益排序（最高）
        by_return = sorted(experiences,
                          key=lambda x: x.pnl_pct, reverse=True)
        
        # 按风险调整收益排序
        by_risk_adj = sorted(experiences,
...
class ExperiencePoolManager:
    ...
    def update_pool(self, experiences: List[Experience],
                   current_phase: str) -> List[Experience]:
        """
        更新经验池
        ...
        """
        # 1. 过滤低衰减因子的经验
        filtered = self.selective_updater.filter_by_decay(
            experiences, current_phase, min_decay=0.1
        )
    ...
    def get_decay_report(self, experiences: List[Experience],
        ...
        Returns:
            衰减报告
        """
        decay_results = self.selective_updater.apply_decay_to_weights(
            experiences, current_phase
        )
...
# 便捷函数
def create_selective_updater(config: Optional[DecayConfig] = None) -> SelectiveUpdater:
    """创建选择性更新器"""
...
def calculate_experience_decay(experience: Experience, 
                              current_phase: str) -> float:
    """快速计算经验衰减因子"""
