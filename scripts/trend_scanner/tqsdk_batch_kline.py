...
def fetch_batch_klines(symbols: list, days: int = 120, period: str = "daily") -> dict:
    ...
    try:
        ...
        # 交易所映射
        SHFE = {'RB', 'HC', 'RU', 'NI', 'CU', 'AL', 'ZN', 'PB', 'SN', 'AU', 'AG', 'BU', 'FU', 'SP', 'SS', 'BR', 'AO', 'WR', 'OP'}
        DCE = {'I', 'J', 'JM', 'M', 'Y', 'P', 'C', 'CS', 'A', 'B', 'JD', 'LH', 'PP', 'V', 'L', 'EG', 'EB', 'PG', 'BB', 'FB', 'RR', 'LG'}
        CZCE = {'CF', 'SR', 'TA', 'MA', 'OI', 'RM', 'SA', 'PF', 'SF', 'SM', 'AP', 'CJ', 'UR', 'ZC', 'PK', 'PX', 'PR', 'EC', 'CY', 'SH'}
        INE = {'SC', 'LU', 'NR', 'BC'}
        CFFEX = {'IF', 'IC', 'IH', 'IM', 'T', 'TF', 'TL', 'TS'}

        def get_exchange(code: str) -> str:
            if code in SHFE: return 'SHFE'
            if code in DCE: return 'DCE'
            if code in CZCE: return 'CZCE'
            if code in INE: return 'INE'
            if code in CFFEX: return 'CFFEX'
            return 'SHFE'
        ...
        for sym, (tq_sym, klines) in kline_objs.items():
            try:
                if klines is None or len(klines) == 0:
                    results[sym] = {'success': False, 'error': f'无数据', 'data': None}
                    continue
